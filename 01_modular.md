...
2つのテータ関数の積は $a_1,a_2,b_1,b_2∈ℝ$, $c_1,c_2∈ℤ$ に対して
$$
\begin{aligned}
...
$$
{#eq:theta_product}
ただし, $a_3≡a_1+a_2$, $b_3≡b_1+b_2$, $c_3≡c_1+c_2$ (どれも mod の意味でない) とした. また $d$ はフラックスの最大公約数 $d≡\gcd(c_1,c_2)$.
...
$$
ψ^{(j+α_1,α_2),M}(z,τ)
  = e^{\frac{πi M}{\Im τ} z \Im z} \mathcal{N}_i ϑ
  \begin{bmatrix}
    \frac{j+α_1}{M} \\
    -α_2
  \end{bmatrix}
...
ψ^{(j+α_1,α_2),M}(z,τ) × ψ^{(k+β_1,β_2),N}(z,τ)
  = ∑_{m}^{P/d-1} Y_{j,k,α,β,m}^{M,N,P}(τ) ψ^{(j+k+mM+γ_1,γ_2),P}(z,τ),
$$
ただし $γ≡α+β$ (mod の意味でない), $P≡M+N$, $d≡\gcd(M,N)$ として, 係数は
$$
...
$$
また, $0≤α,β<1$ と取れば $0≤γ<2$ となることに注意.
...
d^{(jα)(kβ)(ℓγ)}
  ...
  &= ∑_{m}^{(M+N)/d-1} Y_{j,k,α,β,m}^{M,N,(M+N)}(τ) \\
  & \qquad \qquad × ∫_{T^2} dzd\=z ψ^{(j+α_1+k+β_2+mM,α_2+β_2),(M+N)}(z,τ) \pqty{ψ^{(ℓ+γ_1,γ_2),P}(z,τ)}^* \\
  &= ∑_{m}^{(M+N)/d-1} Y_{j,k,α,β,m}^{M,N,(M+N)}(τ) (\Im τ)^{-1/2} e^{2πi\pqty{\frac{(j+α_1+k+β_2+mM)(α_2+β_2)}{M+N} - \frac{(ℓ+γ_1)γ_2}{P}}} \\
  & \qquad \qquad × δ_{M+N,P} δ_{(j+α_1)+(k+β_1)-(ℓ+γ_1),-mM}^{({\rm{mod}}\ M+N)}. \\
\end{aligned}
$$
{#eq:yukawa}
ただし, $M+N=P$ を仮定. また, SS phase について
$$
α + β = γ \pmod1
$$
{#eq:rule_SS}
...
$$
(j+α_1)+(k+β_1)-(ℓ+γ_1) = -mM \pmod{M+N}
$$
の時で, これを満たす $m$ が存在する条件は, $\gcd(M+N,M)=\gcd(N,M)=d$ より
$$
(j+α_1)+(k+β_1)-(ℓ+γ_1) = 0 \pmod d.
$$
あるいは
$$
j + k - ℓ = - (α_1 + β_1 - γ_1) \pmod d.
$$
{#eq:rule_jkl}

### T 変換

波動関数のT変換は, $m = M \pmod 2$ を使って
$$
\begin{aligned}
  T:ψ^{(j+α_1,α_2),M}(z,τ)
    &→ ψ^{(j+α_1,α_2),M}(z,τ+1) \\
...
新しい基底 $Ψ^{iα,M}$ を取ったとき, 3-point coupling ([@eq:yukawa]) が non-zero になる条件を考える. SS-phase の制限 $α + β = γ \pmod 1$ ([@eq:rule_SS])について, $(α_1β_1γ_1)$ の組は $(000)$, $(0\frac12\frac12)$, $(\frac12\frac12)$ の3通りのみだから, その上で $α_2$, $β_2$ を適当に振ったときに制限を満たす項を持つのは以下の24通り.
...
$$
j + k - ℓ = 0 \pmod \gcd(M, N)
$$
は既知であり, 最下段
$$
j + k - ℓ = -1 \pmod \gcd(M, N)
$$
が新しい選択則?
...
:$M = {\rm even}$ のときに[@eq:rule_SS]を満たす SS-phase の組み合わせと $jkℓ$ の条件([@eq:rule_jkl])
...
3. $(k, m_2, ℓ) → (m_1, m_2, ℓ) ∈ ℤ × ℤ × \{0, 1, ..., C'-1\}$: ここまでの変数変換を $N$, $\~N$ に代入すると
    ...
    $$
4. $(m_1, m_2, ℓ) → (m_1, m_2, r) ∈ ℤ × ℤ × \{0, 1, ..., C'-1\}$: $r ≡ (p_{10} - p_{20}) ℓ \pmod C'$ とする. つまり, ある整数 $s$ が存在して,
    $$
